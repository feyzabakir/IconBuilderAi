...
public class UsersAuthController: ControllerBase
{
    ...
    private readonly string _googleAuthorization;

    public UsersAuthController(ISender mediatr, IOptions<GoogleSettings> googleSettings)
    {
        _mediatr = mediatr;
        _googleSettings = googleSettings.Value;
        _googleAuthorization = $"http://accounts.google.com/o/oauth2/v2/auth?" +
    ...
    public async Task<IActionResult> SignInGoogleAsync(string code, CancellationToken cancellationToken)
    {
        ...
        var redirectUrl = $"http://localhost:5262/social-login/social-login?{query}";
